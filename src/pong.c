...
void handle_input_pong(struct Pong* pong, ALLEGRO_KEYBOARD_STATE* state)
{
    ...
    else if (pong->state == PLAY_IA) //Funcionamiento del paddle2 IA
    {
        if (al_key_down(state, ALLEGRO_KEY_S))
        {
            pong->player1.vy = PADDLE_SPEED;
...
{   
    ...
    if (((paddle_index == 1 && ball->vx < 0.0f) || (paddle_index == 2 && ball->vx > 0.0f)) && dx > 17.5) 
    {
        ...
        else if (paddle->y + PADDLE_HEIGHT/2 < ball->y + BALL_SIZE/2)
        {
            paddle->vy = PADDLE_SPEED;
        }
        else 
...
void update_pong(struct Pong* pong, double dt)
{
    if (pong->state == PLAY || pong->state == PLAY_IA || pong->state == PLAY_BIIA)
    {
        ...
        if (ball_hitbox.x1 > TABLE_WIDTH) //player1 marca punto
        {
...
void render_pong(struct Pong pong, struct Fonts fonts)
{
    ...
    else if (pong.state == DONE)
    {
        ...
        sprintf(winner_message, "Player %d win!", pong.winning_player);
